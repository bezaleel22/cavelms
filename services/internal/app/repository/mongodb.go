...
// FetchByID fetches User by array of ids
func (db *mongodb) FetchByIDs(m interface{}, ids []string) error {
	col, _, err := utils.InitCollection(m, db.MongoDB)
	if err != nil {
		return err
	}

	err = col.FindOne(context.TODO(), bson.M{"_id": bson.M{"$in": ids}}).Decode(m)
	if err != nil {
		return err
	}
...
// FetchWhere: fetch where field and value matches
func (db *mongodb) FetchManyWhere(ml interface{}, m interface{}, key string) error {
	col, v, err := utils.InitCollection(m, db.MongoDB)
	if err != nil {
		return err
	}

	cursor, err := col.Find(context.TODO(), bson.M{key: v[key].(string)})
	if err != nil {
		return err
	}
	if err = cursor.All(context.TODO(), ml); err != nil {
...
// UpdateOneWhere:uodate one where field
func (db *mongodb) UpdateOneWhere(m interface{}, field string) error {
	col, v, err := utils.InitCollection(m, db.MongoDB)
	if err != nil {
		return err
	}
	...
	filter := bson.M{"_id": v["_id"]}
	update := bson.M{"$set": bson.M{field + ".$[element]": val[field]}}
...
// UpdateManyWhere: updates many where field and value matches
...
func (db *mongodb) DeleteMany(m interface{}, ids []string) error {
	col, _, err := utils.InitCollection(m, db.MongoDB)
	if err != nil {
		return err
	}

	_, err = col.DeleteMany(context.TODO(), bson.M{"_id": bson.M{"$in": ids}})
	if err != nil {
		return err
	}
	return nil
