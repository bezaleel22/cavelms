<script lang="ts">
  ...
  import type { DatePickerAction } from "./utils";
...
<div tabindex="-1" class="card h-full w-96 bg-neutral text-neutral-content">
  <div class="card-body items-center text-center">
    ...
    >
      <div class="h-14 px-3 py-2 flex items-center w-full">
        <button
          tabindex="-1"
          class="btn btn-xs btn-circle mr-1 bg-default text-default-content border-none outline-none"
        ...
        <button
          tabindex="-1"
          class="btn btn-xs btn-circle bg-default text-default-content border-none outline-none"
        ...
        <button
          tabindex="-1"
          class="btn btn-xs btn-circle ml-1 bg-default text-default-content border-none outline-none"
      ...
      {#if actions.length > 0}
        ...
        >
          {#each actions as action}
            <button class="btn btn-primary bg-default text-default-content" on:click={action.action}
              >{action.label}</button
            >
          {/each}
        </div>
      ...
      {#if showTime || !closeOnSelect}
        <div class="p-3 gap-3 flex items-center justify-evenly w-full border-t border-border">
          <button class="btn w-full bg-default text-default-content" on:click={handleCancel}
            >Clear</button
          >
          <button class="btn btn-primary w-full" on:click={handleApply}>Apply</button>
        </div>
      {/if}
    </div>
  </div>
