...
var help_text: any = {};
function showUsage(help_file: any, headers: any, response: any) {
  ...
  if (help_text[help_file] != null) {
    ...
    require("fs").readFile(help_file, "utf8", function (err: any, data: any) {
      if (err) {
        console.error(err);
 ...
 */
function isValidHostName(hostname: any) {
  ...
  var proxyOptions: any = {
    ...
    buffer: {
      pipe: function (proxyReq: any) {
        ...
        proxyReq.on = function (eventName: any, listener: any) {
...
function onProxyResponse(proxy: any, proxyReq: any, proxyRes: any, req: any, res: any) {
  var requestState: any = req.corsAnywhereRequestState;
...
function getHandler(options: any, proxy: any) {
  ...
  if (corsAnywhere.requireHeader) {
    if (typeof corsAnywhere.requireHeader === "string") {
      ...
      corsAnywhere.requireHeader = corsAnywhere.requireHeader.map(function (headerName: any) {
        return headerName.toLowerCase();
      });
    }
  }
  var hasRequiredHeaders = function (headers: any) {
    return (
      !corsAnywhere.requireHeader ||
      corsAnywhere.requireHeader.some(function (headerName: any) {
  ...
  return function (req: any, res: any) {
    ...
    if (!hasRequiredHeaders(req.headers)) {
      ...
      res.end(
        "Missing required request header. Must specify one of: " + corsAnywhere.requireHeader
      );
      return;
    }
    ...
    corsAnywhere.removeHeaders.forEach(function (header: any) {
...
export const CorsProxy = (options: any) => ({
  name: "configure-server",
  configureServer(server: ViteDevServer) {
    options = options || {};
    var httpProxyOptions: any = {
    ...
    let proxy = httpProxy.createServer(httpProxyOptions);
    let requestHandler = getHandler(options, proxy);
    ...
    proxy.on("error", function (err, req: any, res: any) {
      ...
      let headerNames = res.getHeaderNames ? res.getHeaderNames() : Object.keys(res._headers || {});
      headerNames.forEach(function (name: any) {
